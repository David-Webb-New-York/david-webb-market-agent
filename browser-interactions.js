/**
 * Backend-agnostic browser-interaction logic, shared by browserbase.js and
 * steel.js. Both export the same shape (hasCreds, withPage,
 * renderAndExtract, interactAndExtract) so every existing caller (bb-probe.js,
 * bb-interactive-probe.js, and the import-*.js adapters) works unmodified
 * regardless of which cloud-browser provider is behind it -- only `withPage`
 * differs (how a session gets created/torn down); the actual navigate/type/
 * click/capture logic lives here once, extracted rather than duplicated when
 * Steel.dev was added as a second backend.
 */
...
function makeRenderAndExtract(withPage) {
  return async function renderAndExtract(url, { waitMs = 8000, captureJson = false, sessionOpts = {} } = {}) {
...
function makeInteractAndExtract(withPage) {
  return async function interactAndExtract(
    baseUrl,
    term,
    {
      searchSelectors = [],
...
module.exports = { notableHeaders, isCandidateApiResponse, makeRenderAndExtract, makeInteractAndExtract };
