...
- **Christie's — closest to working of the unresolved houses; real ~2,015-lot dataset confirmed to exist, but the API is auth-gated in a way not yet cracked.** Two URL-param probes (`?entry=`, `?query=`) rendered real unblocked pages but never triggered the search. Built new tooling (`bb-interactive-probe.js` + `browserbase.js`'s `interactAndExtract()`) to simulate real UI interaction (type + Enter) rather than a URL param — this DID trigger Christie's real search infrastructure: `apim.christies.com/count-client?Keyword=david%20webb...` returned `{"Items":[{"Id":"PastLot","Count":2015},...],"Total":2016}` — confirmed, real, not guessed. The listing endpoint `apim.christies.com/search-client` returned an empty `{"lots":[],...}` in that same run (used `is_past_lots=False`), but its response shape (`lots`, `filters.total`, `total_pages`) is exactly what an adapter needs. Tried both a cold plain `fetch()` and an in-browser `fetch()` from within the same authenticated Browserbase session (reusing whatever cookies the real UI interaction had) with `is_past_lots=True` — **both got 404 `{"statusCode":404,"message":"Resource not found"}`**, identical either way. This strongly suggests the real UI-triggered call carries a header (e.g. an Azure APIM subscription key) injected by Christie's own JS bundle that a bare `fetch()` — even from inside the authenticated page — doesn't replicate, since `browserbase.js` doesn't currently capture *request* headers (only method/URL/postData). Diagnostic scripts kept for a future session: `christies-debug-fetch.js` + `.github/workflows/christies-debug.yml`. Next step would be extracting the real request headers (not just URL/body) from a captured Christie's XHR, which needs a `browserbase.js`/`bb-probe.js` extension this session didn't build.
- **Doyle — 6 attempts across two approaches, unresolved.** URL guessing (3 attempts: `/auctions/search?query=` → generic page; `/search?q=` → real but wrong, Doyle's site-wide CMS/blog search, found a blog post *about* David Webb, zero lot data; `/auction/search?q=` → generic homepage). Interactive typing (3 attempts, using the new `bb-interactive-probe.js`): first attempt on the homepage found no matching selector at all; a full-HTML dump then revealed the real search input (`input.search-st`, confirmed exists) is hidden inside an unopened Bootstrap `collapse` dropdown, not interactable on page load; a follow-up with 9 guessed "open search" trigger selectors still found no visible match (they're likely all `display:none` at desktop viewport — Doyle's dropdown-open button wasn't identified). The same HTML dump also showed a plain, non-JS-gated `<form action="/search/" method="get">` with `input name="q"` — but that's the exact URL already tried in attempt 2, which returned generic CMS search, not lot data. Site sits behind Akamai Bot Manager + reCAPTCHA per its own CookieYes cookie-audit data (neither observed triggering). Not pursued further this session.
- **Freeman's/Hindman — 2 probes, both wrong URL.** `?q=` and `?query=` both rendered a real, substantial (185KB) page at `freemanshindman.com/search`, but titled just the bare hostname with zero "david webb" occurrences and zero search API XHRs — the search never executed either time. Domain is plausibly right (real content rendered), URL path is wrong. Not retried further — would need either UI interaction (like Christie's) or manual inspection of the real search route.
- **Barnebys — 2 probes, both a clean 404.** `/search?q=` and `/search?query=` both hit the site's genuine 404 page (confirmed via a working, cleanly-responding first-party `/api/auth/check` endpoint proving the app itself works normally). Not a bot-block, not a param-name issue — `/search` itself isn't a real route on barnebys.com. Needs the real route found (sitemap, manual browse) before retrying.
- **Phillips — 3 probes; confirmed a real, ongoing site outage, not a bot-block or wrong URL.** All three attempts (initial + two retries, hours apart) got byte-for-byte the same 34,875-byte, fully-branded first-party Phillips maintenance page ("We are currently working to bring our website back online as soon as possible") — real wordmark SVG, real app-store links, real office contacts. No Cloudflare/DataDome/Akamai/PerimeterX markers, zero `<script>` tags. phillips.com's own site was genuinely down at every probe time today; simply needs a retry once it's back, not a different approach.
- New reusable tooling built this round for future investigation: `bb-interactive-probe.js` (type-into-search-box + Enter, for sites where URL params don't trigger search — this is what cracked Christie's search infrastructure), `plain-json-get-test.js` (plain-fetch GET-API replay tester, generic counterpart to `plain-post-test.js`), and `browserbase.js`'s new `interactAndExtract()` export.
