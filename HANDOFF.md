...
- **Phillips — probed twice; genuinely a real, transient site outage, not a bot-block or wrong URL.** The full raw HTML (34.8KB, under the 50KB dump threshold) is a legitimate, fully-branded first-party Phillips maintenance page — real wordmark SVG, real app-store links, real office contacts (`ClientServicesNewYork@phillips.com` etc.) — with copy literally reading "We are currently working to bring our website back online as soon as possible." No Cloudflare/DataDome/Akamai/PerimeterX markers, no challenge script, zero `<script>` tags at all. This means phillips.com's own site was down for maintenance at probe time — worth simply re-trying later rather than treating as blocked or chasing a different URL.
