...
function renderDataQualityCaveats() {
  return [
    ...
    '- **A piece can be listed on more than one platform** (e.g. a dealer\'s own site and 1stDibs) — "total listings" and the combined asking-price total above count platform presence, not unique physical pieces. A text/price-matching spot-check (2026-08-12) found ~0.5–1% of listings duplicated *within* the dealers we already track. That undercounts the real picture: a deeper check tracing 5 Back Vault listings via reverse-image search confirmed 4 of 5 were also listed elsewhere — including on a dealer (Greenleaf & Crosby) we weren\'t tracking at all — so the true duplication rate, especially for The Back Vault (61% of active inventory, which discloses no consignor per listing), is likely meaningfully higher than 1%. A systematic image-hash comparison across the full active dataset is in progress to get a firmer number; treat current totals as an upper bound, not a precise count of unique pieces.',
